...
app.get('/product', async (req, res) => {
  console.log('object')
  try {
    const select = await connection.query('SELECT pdt.slug, pdt.id, pdt.name, pdt.price, pdt.description, pdt.image, pdt.quantity, pdt.duration_effect, pwr.power, afEff.effect FROM product pdt JOIN power pwr ON pwr.id = pdt.power_id JOIN after_effect afEff ON afEff.id = pdt.after_effect_id');
